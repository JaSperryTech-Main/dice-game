  ...
  const onRoll = async () => {
    ...
    const animationPromises = currentDices.map((_, index) => {
      return new Promise((resolve) => {
        ...
        // Improved rolling effect: slow and natural-looking
        const rollInterval = setInterval(() => {
          if (rollingDone) return;
          diceElement.textContent = Math.floor(Math.random() * sides) + 1;
        }, 50); // Slower and more readable roll effect

        const baseDuration = Math.random() * (5 - 2) + 2; // 2 to 5 seconds
        ...
        setTimeout(() => {
          ...
          diceElement.classList.remove('rolling'); // Remove animation class
          diceElement.textContent = randomResults[index].result; // Set final roll
          resolve();
        }, duration);
      });
    });
